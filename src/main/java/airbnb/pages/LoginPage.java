 ...
 * the account menu.
 *
 * <p>Airbnb uses a two-step flow:
 * <ol>
 *   <li>Enter email address and click <em>Continue</em></li>
 *   <li>Enter password and click <em>Log in</em></li>
 * </ol>
 *
 * <p>Some UI variants show a list of auth providers first ("Continue with
...
public class LoginPage extends BasePage {
     ...
     *
     * <p>Airbnb renders this as a {@code <button>}, {@code <a>}, or
     ...
     *
     * <p>Airbnb's current login form uses a single {@code input[type='tel']}
     ...
     *
     * <p>Airbnb's login page renders the email input as hidden behind a
     ...
     *
     * <p>Must NOT contain selectors that also match in the logged-out state.
     ...
     *
     * <p>If neither the email field nor a provider button is found within 10 s,
    ...
    public void selectEmailOption() {
        ...
        try {
            ...
            WebElement btn = new WebDriverWait(driver, Duration.ofSeconds(10)).until(d -> {
                ...
                if (!d.findElements(EMAIL_INPUT).isEmpty()) return d.findElements(EMAIL_INPUT).get(0);
            ...
            if (btn.getTagName().equalsIgnoreCase("input")) return;
            btn.click();
            ...
            try {
                new WebDriverWait(driver, Duration.ofSeconds(10)).until(
                    d -> !d.findElements(EMAIL_INPUT).isEmpty());
            } catch (TimeoutException ex) { /* enterEmail() will surface the failure */ }
        }
    }

    /** Types the given email address into the email field.
     *
     * <p>Airbnb's login page uses a floating-label pattern: the
     ...
     *
     * <p>If Airbnb sent a verification code instead of showing the password
    ...
    public void selectPasswordOption() {
        ...
        WebElement found;
        try {
            found = new WebDriverWait(driver, Duration.ofSeconds(15)).until(d -> {
                ...
                if (!otpInputs.isEmpty()) return otpInputs.get(0);
            ...
            return; // nothing appeared in 15 s — let enterPassword() surface the error
        }
        ...
        try {
            new WebDriverWait(driver, Duration.ofSeconds(15)).until(d -> {
                ...
                if (!d.findElements(POST_LOGIN_INDICATOR).isEmpty()) return Boolean.TRUE;
                return null;
            });
        } catch (TimeoutException e) {
     ...
     *
     * <p>If the password input is no longer present or clickable (because an
    ...
    public void enterPassword(String password) {
        try {
            WebElement input = WaitUtils.waitForClickable(driver, PASSWORD_INPUT);
            input.clear();
            input.sendKeys(password);
        } catch (TimeoutException e) {
            // Password input disappeared — likely because Airbnb auto-completed the
            // OTP while we were waiting.  Give the post-login indicator up to 15 extra
            // seconds to appear (the page might still be mid-redirect/loading).
            try {
                new WebDriverWait(driver, Duration.ofSeconds(15)).until(d ->
                    !d.findElements(POST_LOGIN_INDICATOR).isEmpty()
                );
                return; // OTP auto-login confirmed via post-login indicator
            } catch (TimeoutException ignored) {
                // Avatar still not visible — login genuinely failed
            }
            throw e;
        }
    }
     ...
     *
     * <p>Falls back to a JavaScript click when the button is intercepted by
