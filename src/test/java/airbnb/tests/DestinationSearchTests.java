...
public class DestinationSearchTests extends BaseTest {
    ...
    @Test(groups = {FrameworkConstants.GROUP_FUNCTIONAL},
          description = "Clicking the destination field activates (focuses) it")
    ...
    @Test(groups = {FrameworkConstants.GROUP_FUNCTIONAL},
          description = "Typing a valid city name is accepted without errors")
    ...
    @Test(groups = {FrameworkConstants.GROUP_FUNCTIONAL},
          description = "Auto-suggest dropdown appears after typing a destination")
    ...
    @Test(groups = {FrameworkConstants.GROUP_FUNCTIONAL},
          description = "Destination field can be cleared")
    ...
    @Test(groups = {FrameworkConstants.GROUP_FUNCTIONAL},
          description = "Selecting a suggestion moves the flow forward (date picker or next step opens)")
    public void tc005_selectingSuggestionMovesForward() {
        ...
        // After selecting, we expect either a date picker to open or the field value to update
        // — either way the page should not have navigated away yet
        Assert.assertEquals(driver.getCurrentUrl().contains("airbnb.com"), true,
                "Should remain on airbnb.com domain after selecting suggestion");
    }
}
